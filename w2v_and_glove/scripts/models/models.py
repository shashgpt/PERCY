...
class IKD(Model):
    ...
    def train_step(self, data): # an iteration
        ...
        with tf.GradientTape() as tape: # Forward propagation and loss calculation
            ...
            class_object = Teacher_network(batch_size = MINI_BATCH_SIZE, classes = 1, rules = rules, rules_lambda = [1.0], teacher_regularizer = 1.0)
...
def lstm(config, word_vectors):
    ...
    out = layers.Embedding(word_vectors.shape[0], 
                            word_vectors.shape[1], 
                            embeddings_initializer=Constant(word_vectors), 
                            trainable=config["fine_tune_word_embeddings"], #FINE_TUNE_WORD_EMBEDDINGS
                            mask_zero=config["embedding_mask"],
                            name="word_embeddings")(input_sentence)

    # Classifier Layer
    out = layers.LSTM(config["hidden_units_seq_layer"], dropout=config["dropout"], name="classifier")(out)
...
def attention(config, word_vectors):
    global MINI_BATCH_SIZE
    MINI_BATCH_SIZE = config["mini_batch_size"]
