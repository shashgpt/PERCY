...
class Local_lipschitz_estimate(object):
    ...
    def vectorize(self, sentences):
        ...
        character_ids = np.array(batch_to_ids(tokenized_texts))
    ...
    def estimate_discrete_dataset_lipschitz(self, dataset, explanations, probabilities, eps = None, top_k = 1, metric = 'euclidean', same_class = False):
        ...
        den_dists = pairwise_distances(Xs[:,:,0], metric = metric) # Or chebyshev?
        if eps is not None:
    ...
    def lime_explanations_estimates(self):
        ...
        for lime_exp in lime_explanations:
            if len(lime_exp) > max_len:
                max_len = len(lime_exp)
        lime_explanations_padded = []
        for lime_exp in lime_explanations:
            lime_exp = torch.tensor(np.array(lime_exp))
            pad_len = max_len - len(lime_exp)
            exp_padded = torch.nn.functional.pad(lime_exp, (0, pad_len), mode='constant', value=0.0).numpy().tolist()
            lime_explanations_padded.append(exp_padded)
        lime_explanations_padded = np.array(lime_explanations_padded)
        ...
        with open("assets/lime_explanations/"+self.config["asset_name"]+".pickle", "wb") as handle:
            explanations = explanations.to_dict()
