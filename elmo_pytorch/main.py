...
if __name__=="__main__":
   ...
   # Create input data
    ...
    # Evaluate model (asset 2: create results)
    ...
    # LIME explanations (asset 3: create lime explanations)
    ...
    if config["generate_shap_explanations"] == True:
        print("\nSHAP explanations")
        if config["validation_method"] == "early_stopping":
            Shap_explanations(config).create_explanations(train_dataset, test_dataset)
        elif config["validation_method"] == "nested_cv":
            Shap_explanations(config).create_explanations_nested_cv(datasets_nested_cv)
