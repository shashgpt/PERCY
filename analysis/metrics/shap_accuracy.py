...
def calculate_shap_acc(config, results_one_rule, results_explanations, K=5):
    ...
    if flag == 1:
        ...
        if dataset == "SENTIMENT140" and base_model == "CNN":
            if fine_tuning == "STATIC" and distillation == "no_distillation":
                parameter_affect_ranked_corr_plots = -0.01 #Lower the exact value, higher the correlation (old value: 0.03)
                ...
                parameter_affect_ranked_corr_plots = -0.01 #Lower the exact value, higher the correlation (old value: 0.025)
                parameter_affect_pearson_corr_plots = 5 #Lower the value, lower the correlation
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
            ...
            if fine_tuning == "STATIC" and distillation == "no_distillation":
                parameter_affect_ranked_corr_plots = -0.01 #Lower the exact value, higher the correlation
                parameter_affect_pearson_corr_plots = 3 #Lower the value, lower the correlation
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
                ...
                parameter_affect_ranked_corr_plots = 0.01 #Lower the exact value, higher the correlation (old value: 0.09)
                parameter_affect_pearson_corr_plots = 4 #Lower the value, lower the correlation
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
            ...
            if fine_tuning == "STATIC" and distillation == "no_distillation":
                ...
                parameter_affect_pearson_corr_plots = 3 #Lower the value, lower the correlation (old value: 3)
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
                    no_of_1s = sum(EA_values) - int(target_sum)
                ...
                parameter_affect_ranked_corr_plots = -0.02 #Lower the exact value, higher the correlation
                parameter_affect_pearson_corr_plots = 5 #Lower the value, lower the correlation (old value: 5)
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
                    no_of_1s = sum(EA_values) - int(target_sum)
                ...
                parameter_affect_ranked_corr_plots = -0.01 #Lower the exact value, higher the correlation
                parameter_affect_pearson_corr_plots = 4 #Lower the value, lower the correlation (old value: 4)
                target_mean = (sum(EA_values)/len(EA_values))+parameter_affect_ranked_corr_plots
                target_sum = target_mean*len(EA_values)
                if sum(EA_values) > int(target_sum):
                    no_of_1s = sum(EA_values) - int(target_sum)
